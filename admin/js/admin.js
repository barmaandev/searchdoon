jQuery(document).ready(function($) {
    ...
    // Reset all normalized meta
    $('#bwd-reset-normalized').on('click', function() {
        var $button = $(this);
        if (confirm('آیا مطمئن هستید که می‌خواهید تمام داده‌های نرمالایز محصولات را حذف کنید؟\nاین عملیات قابل بازگشت نیست.')) {
            $button.prop('disabled', true).text('در حال حذف...');
            $.ajax({
                url: bwd_ajax.ajax_url,
                type: 'POST',
                data: {
                    action: 'bwd_reset_normalized',
                    nonce: bwd_ajax.nonce
                },
                success: function(response) {
                    if (response.success) {
                        alert((response.data && response.data.message) || 'حذف با موفقیت انجام شد');
                        location.reload();
                    } else {
                        alert('خطا در حذف داده‌های نرمالایز');
                    }
                },
                error: function() {
                    alert('خطا در ارتباط با سرور');
                ...
                complete: function() {
                    $button.prop('disabled', false).text('حذف داده‌های نرمالایز همه محصولات');
                }
            });
        }
    });
